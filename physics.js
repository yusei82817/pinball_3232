...
export function createBall() {
    ball = { x: BALL_START_X, y: BALL_START_Y, vx: 0, vy: 0, radius: BALL_RADIUS };
    ballActive = false;
    return ball;
}
...
export function createWall(x, y, z, width, height, depth) { return { x, y, width, height }; }
...
export function createFlipper(x, y, side) {
    const flipper = { x, y, side, length: 100, width: 22, currentAngle: 0, targetAngle: 0, speed: 0.32 };
...
export function setFlipperTarget(side, pressed) {
    ...
    if (!flipper) return;
    flipper.targetAngle = pressed ? (side === "left" ? -0.95 : 0.95) : 0;
}
...
function updateFlippers() {
    for (const side of ["left", "right"]) {
        ...
        if (Math.abs(difference) < 0.002) flipper.currentAngle = flipper.targetAngle;
    }
}
...
function collideWithWalls() {
    ...
    if (ball.x < minX) { ball.x = minX; ball.vx = Math.abs(ball.vx) * WALL_BOUNCE; }
    if (ball.x > maxX) { ball.x = maxX; ball.vx = -Math.abs(ball.vx) * WALL_BOUNCE; }
    if (ball.y < minY) { ball.y = minY; ball.vy = Math.abs(ball.vy) * WALL_BOUNCE; }
}
...
function collideWithBottom() {
    if (!ball || ball.y < 820) return;

    // 中央240px（x=180～420）がドレイン。
    if (ball.x < 180 || ball.x > 420) {
...
export function stepPhysics() {
    if (!ball) return;
    updateFlippers();
    for (const bumper of bumpers) if (bumper.flash > 0) bumper.flash--;
